//@ sourceURL=AirStatusTrack.js
//状态跟踪
var AirStatusTrack = (function(){
	...
	//	if(html=="AIRexport"){
            ...
            success : function(data) {
                ...
                $("."+html+" #tracing div#jzx div.transportOrder a").remove();
            ...
            success:function (data) {
                ...
                if(data.biz_result.trackList instanceof Array){
                    if(data.biz_result.trackList[0]){
                        $("."+html+" div#tracing div#jzx table.table_air_status tbody>tr").remove();
                        ...
                        var $TR1='<tr class="appendNew" >'+
                            ...
                            '</td>'+
                            '</tr>';
                        $("."+html+" div#tracing div#jzx table.table_air_status tbody").append($TR1);
                    }else{
	...
	function getNumInfosByBusinessId(billExportId,html,childUrl){
		$.ajax({
			...
			success : function(data) {
				...
				/*报关单号
				if($("."+html+" #tracing div#tg div.customId a").length <= 0){
				$("."+html+" #tracing div#tg div.customId").append('<a name=customId>'+data.billLandingNo+'</a>');
				}*/
                $("."+html+" #tracing div#tg div.billLandingId a").remove();
				//if($("."+html+" #tracing div#tg div.billLandingId a").length <= 0){
					$("."+html+" #tracing div#tg div.billLandingId").append('<a name=billLandingId>'+data.billLandingNo+'</a>');
				//	}
            }
		})	
	}
		...
		$.ajax({
            ...
            success:function (data) {
                if(data.error){
                	...
                	//为下面的箱子赋值
                    $('.'+html+' div#tracing div#tg table.table_1 tbody>tr').remove();
                    $('.'+html+' div#tracing div#tg table.table_2 tbody>tr').remove();
                    var $strTR;
                    ...
                    $('.'+html+' div#tracing div#tg table.table_1 tbody').append($strTR);
                    var $TR='';
                    for(var i = 0; i < data.biz_result[0].clearingStatusInformation.length; i++) {
                    ...
                    $('.'+html+' div#tracing div#tg table.table_2 tbody').append($TR);
                }
            }
		})
...
$(function(){
	//空运出口航班信息
	$('.AIRexport div.trace').on('click','>ul>li',function(){
		var id = $(".AIRexport ul.Big").find('li.active').attr('billId');
		AirStatusTrack.getNumInfo(id,"AIRexport");
	})
	$('.AIRexport #tracing div#jzx div.transportOrder').on('click','a',function(){
		var conNum=$(this).text();
        AirStatusTrack.askDetail(conNum,"AIRexport");
	})
	
	$('.AIRexport div#myTabContent div#exportStatusTrack li.tg').on('click',function(){
		var id = $(".AIRexport ul.Big").find('li.active').attr('billId');
		AirStatusTrack.getNumInfosByBusinessId(id,"AIRexport","airBillExportController/searchById.do");
	})
	/*
	    报关单号
	$('.AIRexport #tracing div#tg div.customId').on('click','a',function(){
		var conNum=$(this).text();
        AirStatusTrack.getClearanceInfo(conNum,"","AIRexport");
	})*/
	$('.AIRexport #tracing div#tg div.billLandingId').on('click','a',function(){
		var labNum=$(this).text();
        AirStatusTrack.getClearanceInfo("",labNum,"AIRexport");
	})
