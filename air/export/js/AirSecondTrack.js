//@ sourceURL=AirSecondTrack.js
//二程跟踪
var AirSecondTrack = (function() {
	var data = {};
	...
	function askData(billId, businessType,html) {
		$.ajax({
					...
					success : function(data) {
						...
						AirSecondTrack.data['airTrackSecondParamVos'] = data['aaData'];
	...
	function saveData(billId,businessType,html) {
		// 修改数据
		 ...
		 console.log(tr);
		 console.log($(tr));
		 AirSecondTrack.data['airTrackSecondParamVos'][i]['exchange_operate']=$(tr).attr('exchange_operate');
		 $.each($(tr).find('input'),function(j,input){
		 console.log($(this).attr('name'));
		 var name=$(this).attr('name');
		 console.log(i);
		 console.log(AirSecondTrack.data['airTrackSecondParamVos'][i])
		 console.log(AirSecondTrack.data['airTrackSecondParamVos'][i][name]);
		 if(AirSecondTrack.data['airTrackSecondParamVos'][i][name]){
		 AirSecondTrack.data['airTrackSecondParamVos'][i][name]=$(this).val();
		 }
		 	})
		 })
		//新增的数据
			$.each($('.'+html+' div#tracing div div#exportSecondTrack form table').find('tr[exchange_operate="insert"]'),function(i,tr){
				...
				console.log("--------")
				console.log(AirSecondTrack.data);
				AirSecondTrack.data['airTrackSecondParamVos'].push(obj);
			});
		 	console.log("新增的数据");
			console.log(AirSecondTrack.data);
			//AirSecondTrack.data['billId']=billId;
			 $.ajax({
	             type:'post',
	             url:'/SeawinWebappBase/airTrackSecondController/updateById.do',  
	             contentType: "application/json;charset=UTF-8",
	             data:JSON.stringify(AirSecondTrack.data),
	             success:function(data){
	            	 console.log(data);
...
$(function() {
	$('.AIRexport div#tracing  div div#exportSecondTrack form table')
